...
module.exports = {
    deleteStoreData: function (article_id) {
        return new Promise(async (resolve, reject) => {
            await dbCURD.deleteStoreData(article_id);
            resolve();
        });
    },
    ...
    asyncCollectById: async function (itemUrl, article_id) {
        console.log(itemUrl);
        let downloadData = await httpHandle.getDownload(itemUrl);
        if (downloadData) {
            ...
            $("a").each(async function (i, e) {
                ...
                if (reg.test($(e).attr("href"))) {
                    await asyncGetComments($(e).attr("href"), article_id);
                }
            });
        }
    }
...
let asyncGetComments = async function recursiveComments(commentsUrl, article_id) {
    console.log(commentsUrl);
    let commentData = await httpHandle.getDownload(commentsUrl);
    if (commentData) {
        //获取当页评论
        ...
        $originData(".revRvwUserSec").each(function (i, originData) {
            async function asyncCollectComments(originData) {
            ...
            asyncCollectComments(originData);
        });
        // 查找是否有下一页入口, 有则递归调用本函数
        $originData("a").each(async function (i, e) {
            let reg = /^(次の15件)(.*)$/gi;
